...
<div class="entry-line cfreviewp-meta-{{safeStringify meta_key}}">
  ...
  <div>
    ...
    {{#ifEquals meta_key "Review Status"}}
      <button id="cfreviewp-button-accept" class="ajax-trigger" onClick="baldrickTriggers()" data-entry="{{entry_id}}" data-nonce="<?= $nonce ?>" data-action="cfreviewp_review_entry" data-response="1" data-request="cfreviewp_review" type="button" {{#ifEquals meta_value "ACCEPTED"}}style="display: none;"{{/ifEquals}}>Accept</button>
